...
import numpy as np
from ._batch import CovBatch
from ._dataset import CovDataset
import wandb
...
class CovDataModule(LightningDataModule):
    ...
    def _make_dataloader(self, partition: Literal["train", "val"]) -> DataLoader:
        df = self.dataset[self.dataset.split == partition]
        dataset = CovDataset(df, self.alphabet)
        return DataLoader(
            dataset,
            batch_size=self.batch_size,
            shuffle=partition == "train",
            num_workers=self.num_workers,
            collate_fn=CovBatch.from_tensor_pylist,
        )

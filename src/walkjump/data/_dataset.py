...
class AbDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        tokens = torch.cat([tensor_h, tensor_l])*1.0
        sigma_t = torch.tensor([row.sigma], dtype=tokens.dtype)
        
        return torch.cat([tokens,sigma_t])
...
class CovDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        row = self.df.loc[index]
        tensor_h = token_string_to_tensor(row.heavy_chain, self.alphabet)
        tensor_l = token_string_to_tensor(row.light_chain, self.alphabet)
...
class HERWithLabelDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        tensor = token_string_to_tensor(row.AASeq, self.alphabet)*1.0
        
        label = torch.tensor(row.AgClass).long()
        #print(row.AASeq,label)
        #return tensor, label
       
        sigma_t = torch.tensor([row.sigma], dtype=tensor.dtype)
        
        return torch.cat([tensor,sigma_t]), label
...
class AMPDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        tensor = token_string_to_tensor(seq, self.alphabet)*1.0
        sigma_t = torch.tensor([row.sigma], dtype=tensor.dtype)

        return torch.cat([tensor,sigma_t])

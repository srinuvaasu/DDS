...
class TrainableScoreModel(LightningModule):
    ...
    def training_step(self, batch: AbBatch, batch_idx: int) -> torch.Tensor:
        Sigma = batch.batch_tensor[:,-1]
        batch.batch_tensor = batch.batch_tensor[:,0:-1]
        #print(batch.batch_tensor.shape,Sigma[0:2])
        Sigma = None
        if Sigma!=None:
            self.sigma = Sigma.view(-1, 1, 1)
        #print(batch.batch_tensor.shape,self.sigma)
        loss = self.compute_loss(batch,Sigma)
...
class ClassifierModel(LightningModule):
    ...
    def training_step(self, batch: MNISTWithLabelsBatch, batch_idx: int) -> torch.Tensor:
        #print("trianigni step")
        loss = self.compute_loss(batch)
        batch_size = batch.batch_tensor.size(0)
        self.log("train_loss", loss, sync_dist=True, batch_size=batch_size, rank_zero_only=True)
        return loss

    def validation_step(self, batch: MNISTWithLabelsBatch, batch_idx: int) -> torch.Tensor:
        loss = self.compute_loss(batch)
        batch_size = batch.batch_tensor.size(0)
        self.log("val_loss", loss, sync_dist=True, batch_size=batch_size, rank_zero_only=True)
        return loss
...
class DiscriminatorModel(LightningModule):
    ...
    def training_step(self, batch: AbWithLabelBatch, batch_idx: int) -> torch.Tensor:
        loss = self.compute_loss(batch)
        batch_size = batch.batch_tensor.size(0)
        self.log("train_loss", loss, sync_dist=True, batch_size=batch_size, rank_zero_only=True)
        return loss

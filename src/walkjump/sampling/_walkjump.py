...
import torch
import numpy as np
from walkjump.constants import ALPHABET_AHO,ALPHABET_AMP, LENGTH_FV_HEAVY_AHO, LENGTH_FV_LIGHT_AHO
...
def stack_seed_sequences(seeds: list[str], num_samples: int,alphabet) -> torch.Tensor:
    """
    ...
    return torch.nn.functional.one_hot(
        torch.nn.utils.rnn.pad_sequence(
            [token_string_to_tensor(seed_i, alphabet, onehot=False) for seed_i in seeds],
            batch_first=True,
        ).repeat_interleave(num_samples, dim=0),
        num_classes=len(alphabet.classes_),
    ).float()
...
def walkjump(
    ...
    fv_light_aho_seed_list = token_string_from_tensor(
        seed_tensor[:, :LENGTH_FV_LIGHT_AHO], alphabet, from_logits=True
    )
